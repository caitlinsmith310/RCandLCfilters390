...
zp=Rp/(1+j*Rp*(ang_freq*C-(1/(ang_freq*L))))
T=R2/(R2+zp)
...
plt.title("Multisim Bode diagram, Circuit C")
h,  l  = ax.get_legend_handles_labels()
h2, l2 = ax2.get_legend_handles_labels()
ax.legend(h+h2, l+l2, loc=0)

plt.figure("Multisim Nyquist, Circuit C")
plt.plot(phasere,phaseim, label="Multisim")
plt.title("Multisim Nyquist, Circuit C ")
plt.ylabel("Imaginary")
plt.xlabel("Real")
plt.gca().set_aspect('equal', adjustable='box')
plt.grid()   
